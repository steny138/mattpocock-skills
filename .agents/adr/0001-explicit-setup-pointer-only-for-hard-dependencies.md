...
- **Hard dependency** (`to-tickets`, `triage`) — include an explicit one-liner: _"… should have been provided to you — run `/setup-matt-pocock-skills` if not."_ Without the mapping, output is wrong, not just fuzzy.
- **Fixed local artifact** (`to-spec`, `to-plan`) — write beneath `.scratch/` and use the repository-local Git exclude. Their output location does not depend on issue-tracker setup.
