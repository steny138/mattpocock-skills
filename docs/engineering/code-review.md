...
`code-review` is the review step at the tail of the build chain — `grill-with-docs → to-spec → to-tickets → to-plan → implement → code-review` — and also stands alone on any branch or PR you point it at.

- [implement](https://aihero.dev/skills-implement) is the closest neighbour: it calls this skill as the completion gate after the plan's task commits exist.
- [to-spec](https://aihero.dev/skills-to-spec), [to-tickets](https://aihero.dev/skills-to-tickets), and [to-plan](https://aihero.dev/skills-to-plan) produce the source and fixed point the Spec axis checks against; a vague source makes that axis vague.
