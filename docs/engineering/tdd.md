...
| You have a [spec](https://www.aihero.dev/ai-coding-dictionary/spec) or [tickets](https://www.aihero.dev/ai-coding-dictionary/ticket) and want the whole build run for you | [to-plan](https://aihero.dev/skills-to-plan), then [implement](https://aihero.dev/skills-implement), which drives `tdd` from the approved plan |
...
[to-spec](https://aihero.dev/skills-to-spec) first agrees the feature-level test seams, [to-plan](https://aihero.dev/skills-to-plan) records the seams for the selected work item, [implement](https://aihero.dev/skills-implement) drives `tdd` against them, and [code-review](https://aihero.dev/skills-code-review) checks the resulting commits afterwards. Its other neighbour is [codebase-design](https://aihero.dev/skills-codebase-design), the shared source of the seam and deep-module vocabulary `tdd` speaks. You can also reach for it on its own whenever there is a concrete behaviour to build without the full flow. When you are unsure which skill fits your situation, [ask-matt](https://aihero.dev/skills-ask-matt) routes you.
