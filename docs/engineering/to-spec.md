...
| Decided, and the work fits one [context window](https://www.aihero.dev/ai-coding-dictionary/context-window) | [to-plan](https://aihero.dev/skills-to-plan), then [implement](https://aihero.dev/skills-implement) — skip the spec |
...
Its neighbours upstream are [grill-with-docs](https://aihero.dev/skills-grill-with-docs), which does the deciding this skill only records, and [wayfinder](https://aihero.dev/skills-wayfinder), whose finished map merges onto the chain right here. Downstream, [to-tickets](https://aihero.dev/skills-to-tickets) cuts the spec into tracer-bullet tickets; each selected frontier ticket then receives a disposable local plan from [to-plan](https://aihero.dev/skills-to-plan) before [implement](https://aihero.dev/skills-implement) executes it. When you're unsure which skill or flow fits, [ask-matt](https://aihero.dev/skills-ask-matt) routes you.
