...
It sits between [to-spec](https://aihero.dev/skills-to-spec), which hands it a settled spec with user stories to slice against, and [to-plan](https://aihero.dev/skills-to-plan), which writes the disposable implementation plan for one selected frontier Issue. [implement](https://aihero.dev/skills-implement) then executes that approved plan through [tdd](https://aihero.dev/skills-tdd) and [code-review](https://aihero.dev/skills-code-review). Work the frontier one Issue per fresh context, clearing between them. When you're unsure which skill or flow fits, [ask-matt](https://aihero.dev/skills-ask-matt) routes you.
