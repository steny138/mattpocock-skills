...
`triage` is an **on-ramp**, not a step in the main chain. The main flow runs from an idea you had — grill, spec, tickets, per-ticket plan, implement, review — and `triage` is the parallel lane for work that arrived instead. It merges at the same planning boundary: an issue labelled `ready-for-agent` with a brief on it becomes the source for [to-plan](https://aihero.dev/skills-to-plan), whose approved local plan is then executed by [implement](https://aihero.dev/skills-implement). When a request needs sharpening before it can be briefed, `triage` runs [grilling](https://aihero.dev/skills-grilling) and [domain-modeling](https://aihero.dev/skills-domain-modeling) together, a round of questions at a time, so decisions land in `CONTEXT.md` and the ADRs as they're made. When you're not sure which lane you are in, [ask-matt](https://aihero.dev/skills-ask-matt) routes you.
