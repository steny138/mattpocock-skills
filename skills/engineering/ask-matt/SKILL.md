...
3. **Branch — is this a multi-session build?**
   - **Yes** → **`/to-spec`** (write the thread to `.scratch/<feature>/spec.md`), then **`/to-tickets`** to split it into tracer-bullet tickets, each declaring its **blocking edges**. The spec stays local regardless of tracker choice. On a local tracker that's one file per ticket under `.scratch/<feature>/issues/`, worked blockers-first by hand; on a real tracker the edges become native blocking links, so any ticket whose blockers are done can be grabbed. For one selected frontier ticket, run **`/to-plan`** to write its disposable local implementation plan, confirm it, then run **`/implement`** against that plan. Clear context before selecting the next ticket.
...
Keep steps 1–3 in **one unbroken context window** — don't compact or clear until after `/to-tickets` — so the grilling, local spec, and issues all build on the same thinking. Each frontier issue then gets its own `/to-plan`; its local plan is the resumable seam another agent passes to `/implement`.
...
**`/setup-matt-pocock-skills`** — run before a tracker-dependent engineering flow to configure the issue tracker, triage labels, and doc layout. `/to-spec` and `/to-plan` do not need tracker setup; their output paths are fixed beneath `.scratch/`. Custom issue trackers also work for the skills that use them.
